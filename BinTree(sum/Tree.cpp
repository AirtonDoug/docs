...
int Tree::_min_key(Node *node) { // TODO
    if(node->left == nullptr && node->right == nullptr){
        ...
        if(node->left == nullptr && node->right !=nullptr){
            if(node->key > _min_key(node->right))
                return _min_key(node->right);
            else
                return node->key;        
        }else if(node->left!=nullptr && node->right == nullptr){
            if(node->key > _min_key(node->left))
                return _min_key(node->left);
            else
                return node->key;
        }else{
            if(_min_key(node->right) < _min_key(node->left) && _min_key(node->right) < node->key)
                return _min_key(node->right);
            else  if(_min_key(node->left) < _min_key(node->right) && _min_key(node->left) < node->key)
                return _min_key(node->left);
            else
                return node->key;    
        }
...
int Tree::_total_internal_nodes(Node *node) { // TODO
    if(node == nullptr || node->left == nullptr && node->right == nullptr){
        ...
        if(node->left == nullptr && node->right!=nullptr){
            return 1+_total_internal_nodes(node->right);
         }
        else if(node->left != nullptr && node->right == nullptr){
